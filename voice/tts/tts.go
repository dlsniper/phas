...
func (s Service) Speak(ctx context.Context, text string) {
	...
	for {
		...
		readBytes += n
		// It seems that sometimes the io.EOF is not enough to stop so we need to keep track of the read bytes...
...
func NewService(service *texttospeech.Client) *Service {
	player, err := oto.NewPlayer(24000, 1, 2, 8192)
	if err != nil {
		log.Fatal(err)
	}
